-- ============================================================================
-- Migration 031: Fix Odd Semester Subject Offerings
-- Purpose: Properly add odd semester offerings with CORRECT subject codes
-- Problem: Migration 030 used wrong codes (Sem II codes for Sem I)
-- Solution: Use correct codes from migration 026
-- ============================================================================
...
-- Step 2: Ensure cycles exist for odd semesters
...
-- Step 3: Ensure all required subjects exist (idempotent)
INSERT INTO subject (code, name, l, t, p, credits, tch, course_category) VALUES
-- MCA Semester I (from migration 006)
...
-- MCA Semester III (from migration 006)
...
-- BCA Semester I (from migration 006)
...
-- BCA Semester III (from migration 006)
...
-- BCA Semester V (from migration 006)
...
-- Using correct pattern from migration 026

-- MCA Semester I offerings (7 subjects × 3 sections = 21 offerings)
INSERT INTO subject_offering (subject_id, program_id, semester_id, section_id, shift, student_strength, academic_year, academic_year_id, old_academic_cycle_id)
...
-- BCA Semester III offerings (6 subjects × 6 sections = 36 offerings)
INSERT INTO subject_offering (subject_id, program_id, semester_id, section_id, shift, student_strength, academic_year, academic_year_id, old_academic_cycle_id)
...
CROSS JOIN (SELECT id FROM program WHERE name IN ('BCA(General)', 'BCA(DB)', 'BCA(MM)', 'BCA(Cyber)', 'BCA(DB+MM)', 'BCA(Cyber+MM)')) p
CROSS JOIN (SELECT id FROM section WHERE label IN ('A', 'B', 'C', 'D', 'E', 'F')) sec
WHERE s.code IN ('ACA31010','ACA31009','GLS51005','GGE51015','ABB31001','ACA31800')
ON CONFLICT DO NOTHING;

-- BCA Semester V offerings (6 subjects × 2 programs × 6 sections = 72 offerings)
-- Note: Semester V is only for Cyber specialization programs
INSERT INTO subject_offering (subject_id, program_id, semester_id, section_id, shift, student_strength, academic_year, academic_year_id, old_academic_cycle_id)
...
BEGIN
    ...
    SELECT COUNT(*) INTO v_total_all FROM subject_offering;
    v_total_odd := v_sem1_count + v_sem3_count + v_sem5_count;
    
    RAISE NOTICE '031: Semester I offerings = %', v_sem1_count;
    RAISE NOTICE '031: Semester III offerings = %', v_sem3_count;
    RAISE NOTICE '031: Semester V offerings = %', v_sem5_count;
    RAISE NOTICE '031: Total odd semester offerings = %', v_total_odd;
    RAISE NOTICE '031: Total all offerings = %', v_total_all;
    
    -- Expected counts:
    -- Sem I: 21 (MCA) + 42 (BCA) = 63
    -- Sem III: 15 (MCA) + 36 (BCA) = 51
    -- Sem V: 72 (BCA Cyber only)
    -- Total odd: 186
    
    IF v_total_odd < 100 THEN
        RAISE WARNING '031: Expected ~186 odd semester offerings, got only %. Check subject/program/section existence.', v_total_odd;
    END IF;
END $;
