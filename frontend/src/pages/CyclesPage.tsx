...
export default function CyclesPage() {
    ...
    if (loading) return <div className="page-container"><p style={{ color: 'var(--color-text-muted)' }}>Loading...</p></div>;
        ...
        <div className="page-container">
            ...
            {activeCycle && (
                <div className="glass-card" style={{
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '0.5rem' }}>
                        ...
                        <span className="badge badge-success" style={{ fontSize: '0.875rem', padding: '0.375rem 0.75rem' }}>
                            ACTIVE CYCLE
                        </span>
                    </div>
                    <div style={{ display: 'flex', gap: '2rem', fontSize: '0.875rem', color: 'var(--color-text-muted)' }}>
                        <span><strong style={{ color: 'var(--color-text)' }}>{activeCycle.academic_year}</strong> · {activeCycle.semester_type}</span>
            ...
            {showForm && (
                <div className="glass-card" style={{ padding: '1.5rem', marginBottom: '1.5rem' }}>
                    <h3 style={{ fontSize: '1rem', fontWeight: 600, marginBottom: '1rem' }}>Create New Cycle</h3>
                    ...
                    <tbody>
                        {cycles.map((c) => (
                            <tr key={c.id}>
                                ...
                                <td style={{ fontSize: '0.8125rem', color: 'var(--color-text-muted)' }}>{c.created_at?.slice(0, 10)}</td>
                                <td>
                                    {!c.is_active && (
                                        <button onClick={() => handleActivate(c.id)} className="btn btn-success" style={{ padding: '0.25rem 0.5rem', fontSize: '0.75rem' }}>
