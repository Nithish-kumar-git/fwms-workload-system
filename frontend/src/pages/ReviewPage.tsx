...
export default function ReviewPage() {
    ...
    if (loading) return (
        <div className="page-container">
            <p style={{ color: 'var(--color-text-muted)', textAlign: 'center', padding: '3rem' }}>Loading allocations...</p>
        </div>
    );
    ...
    return (
        <div className="page-container">
            ...
            {allocations.length === 0 ? (
                <div className="glass-card" style={{ padding: '2rem', textAlign: 'center' }}>
                    <p style={{ color: 'var(--color-text-muted)' }}>No allocations found. Run the allocation engine first.</p>
                </div>
            ) : (
                programs.map((prog) => {
                    ...
                    return (
                        <div key={prog} className="mb-8">
                            <h2 className="text-lg font-semibold mb-4 text-blue-600 dark:text-blue-400 pl-1">
                                {prog}
                            </h2>
                            {semesters.map((sem) => {
                                ...
                                return sections.map((sec) => {
                                    ...
                                    return (
                                        <div key={`${prog}-${sem}-${sec}`} className="glass-panel overflow-hidden mb-6">
                                            <div className="px-5 py-3 border-b border-black/5 dark:border-white/5 bg-black/[0.02] dark:bg-white/[0.02] flex gap-3 items-center">
                                                <span className="badge badge-info">{sem}</span>
                                                <span className="badge badge-warning">{sec}</span>
                                                <span className="text-gray-500 dark:text-gray-400 text-[13px] font-medium ml-auto">
                                                    ...
                                                    <tbody>
                                                        {allocs.map((a) => (
                                                            <tr key={a.allocation_id}>
                                                                ...
                                                                <td className="font-mono text-gray-500 dark:text-gray-400 text-[13px]">{a.emp_code}</td>
                                                                <td className="text-gray-700 dark:text-gray-300">{a.subject_code} <span className="text-gray-400 mx-1">—</span> {a.subject_name}</td>
                                                                <td className="text-gray-500">{a.l_assigned}</td>
                                                                <td className="text-gray-500">{a.t_assigned}</td>
                                                                <td className="text-gray-500">{a.p_assigned}</td>
                                                                <td className="font-semibold text-blue-600 dark:text-blue-400">{a.ltp_total}</td>
                                                                <td>
